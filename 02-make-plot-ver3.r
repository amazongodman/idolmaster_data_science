...
Sys.setenv("plotly_api_key"="APIhere")

everyone<-plot_ly(data = data2,
        x = ~weight,
        y = ~hight,
        split = ~roma,
        type = "scatter")
...
api_create(everyone, filename = "everyone-names")
...
###########
fig <- plot_ly(data=data2, x = ~weight, y = ~waist, text = ~roma, type = 'scatter', mode = 'markers',color = ~bust, colors = 'Reds',
               marker = list(size = ~bust, opacity = 0.9))
fig <- fig %>% layout(title = 'weight-waist and bust',
                      xaxis = list(showgrid = FALSE),
                      yaxis = list(showgrid = FALSE))
fig
...
api_create(fig, filename = "weight-waist-bust-bubble")
...
write.csv(unknown_weight, "unknown_weight.csv", fileEncoding = "CP932",row.names=F)
write.csv(unknown_old, "unknown_old.csv", fileEncoding = "CP932",row.names=F)
write.csv(unknown_weight, "unknown_weight.csv", fileEncoding = "CP932",row.names=F)
write.csv(unknown_waist, "unknown_3size.csv", fileEncoding = "CP932",row.names=F)
